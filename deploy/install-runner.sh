...
mkdir -p "$RUNNER_DIR"
cd "$RUNNER_DIR"
# Via gh (box-authenticated): avoids anonymous API rate limits. Asset names
# carry the version (actions-runner-linux-x64-2.336.0.tar.gz), so match the
# versioned pattern, not a literal "linux-x64.tar.gz".
asset_url="$(gh api repos/actions/runner/releases/latest \
  --jq '[.assets[] | select(.name | test("linux-x64-[0-9.]+\\.tar\\.gz$")) | .browser_download_url][0]')"
